    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button 
          ...
          className="flex items-center bg-black dark:bg-white text-white dark:text-black px-5 py-2.5 rounded-xl font-bold text-sm shadow-lg transition-all hover:opacity-80 active:scale-95"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-zinc-900 p-5 rounded-2xl border border-zinc-800 shadow-sm">
          ...
          <p className="text-2xl font-black text-white">{reportData.filter(r => r.status === 'active').length}</p>
        </div>
        <div className="bg-zinc-900 p-5 rounded-2xl border border-zinc-800 shadow-sm">
          <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Gross Annual Rent</p>
          <p className="text-2xl font-black text-zinc-300">₦{reportData.reduce((acc, r) => acc + r.rentAmount, 0).toLocaleString()}</p>
        </div>
        <div className="bg-zinc-900 p-5 rounded-2xl border border-zinc-800 shadow-sm">
          <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">Expiring Soon (&lt;30d)</p>
          <p className="text-2xl font-black text-white">{reportData.filter(r => r.daysRemaining <= 30 && r.status === 'active').length}</p>
        </div>
      </div>
        ...
        <div className="p-4 border-b border-zinc-800 bg-zinc-950 flex flex-col sm:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-10 pr-4 py-2 bg-black border border-zinc-800 rounded-xl text-sm outline-none focus:ring-2 focus:ring-white transition-all text-white"
              value={searchTerm}
            ...
            <tbody className="divide-y divide-zinc-800">
              {reportData.length > 0 ? reportData.map((row, idx) => (
                <tr key={idx} className="hover:bg-black/50 transition-colors group">
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col">
                      <span className="text-xs font-bold text-zinc-300">{row.propertyName}</span>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <span className={`text-xs font-bold ${row.daysRemaining <= 30 && row.status === 'active' ? 'text-white' : 'text-zinc-400'}`}>
                        {row.expiryDate}
                      </span>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex justify-center">
                      <span className={`px-2 py-1 rounded text-[10px] font-bold uppercase ${
                        row.status === 'active' ? 'bg-white text-black' : 'bg-zinc-800 text-zinc-500'
                      }`}>
                        {row.status}
                      </span>
                    </div>
                  ...
                  <td colSpan={7} className="px-6 py-32 text-center">
                    <div className="flex flex-col items-center justify-center opacity-40">
                      <div className="relative mb-6">
                        <Table size={64} className="text-zinc-500" />
                        <Search size={24} className="absolute -bottom-2 -right-2 text-zinc-400" />
                      </div>
                      <h3 className="text-lg font-black text-white uppercase tracking-tighter mb-2">No Records Found</h3>
